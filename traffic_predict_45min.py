...
def get_compressed_representation():
    ...
    traffic_mat = trafficFrame.values                                             #shape(traffic_mat) = (946,19)
    target_traffic = target_trafficFrame.values                                   #shape(target_traffic) = (946,)
    ...
    mat_compressed =  sess.run(ae['z'],feed_dict={ae['x']:traffic_mat,ae['corrupt_prob']: [1.0]}) ##shape(mat_compressed) = (946,2)

    ##generate mat for LSTM inputs
    mat_for_predict = np.column_stack((target_traffic,mat_compressed))                            ## shape(mat_for_predict) = (946,3)
    sess.close()
    return mat_for_predict,target_traffic                                                    ##  shape(target_traffic) = (946,)
...
def get_trainSet():
    ...
    return seq,res,xs                                                           ##shape(seq) = (946-32, 32, 3)
                                                                                ##shape(res) = (946-32, 32, 1)
                                                                                ##shape(xs) = (946-32,)
...
if __name__ =='__main__':
    ...
    writer = tf.train.SummaryWriter("logs_45min", sess.graph)
    sess.run(tf.initialize_all_variables())
...
##test model2                                                                        ##shape(seq) = (946-32, 32, 3)
    seq,res,xs = get_trainSet()                                                      ##shape(res) = (946-32, 32, 1)
    feed_dict = {                                                                     
                model.xs:seq[700:910:3].reshape(-1,TIME_STEPS,3)
                }
    
    res = res[700:910:3].reshape(-1,32,1)                                            ##the ground truth result to compare
    res = res[::10]
