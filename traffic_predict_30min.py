...
def loadDataSet():
    ...
    traffic_dataFrame = pd.read_csv("/home/johnson/tensorflow/row Data/nj06downbsloc30min_new.csv")

    MyTrafficFrame = traffic_dataFrame.reindex(columns=surround_cells)        #[1427 rows * 39 columns]

    ##miss data operation
    MyTrafficFrame =  MyTrafficFrame.dropna(axis = 1,thresh = 1410)           #[1427 rows * 19 columns]
...
def get_compressed_representation():
    ...
    traffic_mat = trafficFrame.values                                             #shape(traffic_mat) = (1427,19)
    target_traffic = target_trafficFrame.values                                   #shape(target_traffic) = (1427,)
    ...
    mat_compressed =  sess.run(ae['z'],feed_dict={ae['x']:traffic_mat,ae['corrupt_prob']: [1.0]}) ##shape(mat_compressed) = (1427,2)

    ##generate mat for LSTM inputs
    mat_for_predict = np.column_stack((target_traffic,mat_compressed))                            ## shape(mat_for_predict) = (1427,3)
    sess.close()
    return mat_for_predict,target_traffic                                                         ##  shape(target_traffic) = (1427,)
...
def get_trainSet():
    ...
    return seq,res,xs                                                           ##shape(seq) = (1427-48, 48, 3)
                                                                                ##shape(res) = (1427-48, 48, 1)
                                                                                ##shape(xs) = (1427-48,)
...
if __name__ =='__main__':
    ...
    writer = tf.train.SummaryWriter("logs_30min", sess.graph)
    sess.run(tf.initialize_all_variables())
...
##test model2                                                                    ##shape(res) = (1427-48, 48, 1) 
    seq,res,xs = get_trainSet()                                                  ##shape(seq) = (1427-48, 48, 3)
    feed_dict = {                                                                                                                                     
                model.xs:seq[1000:1350:7].reshape(-1,TIME_STEPS,3)
                }
    
    res = res[1000:1350:7].reshape(-1,TIME_STEPS,1)                                            ##the ground truth result to compare
    res = res[::7]
