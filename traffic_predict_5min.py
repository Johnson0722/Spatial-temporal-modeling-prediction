...
def loadDataSet():
    ...
    traffic_dataFrame = pd.read_csv("/home/johnson/tensorflow/row Data/nj06downbsloc5min_new.csv")

    MyTrafficFrame = traffic_dataFrame.reindex(columns=surround_cells)        #[8594 rows * 39 columns]

    ##miss data operation
    MyTrafficFrame =  MyTrafficFrame.dropna(axis = 1,thresh = 8500)           #[8594 rows * 19 columns]
...
def get_compressed_representation():
    ...
    traffic_mat = trafficFrame.values                                             #shape(traffic_mat) = (8594,19)
    target_traffic = target_trafficFrame.values                                   #shape(target_traffic) = (8594,)
    ...
    mat_compressed =  sess.run(ae['z'],feed_dict={ae['x']:traffic_mat,ae['corrupt_prob']: [1.0]}) ##shape(mat_compressed) = (8594,2)

    ##generate mat for LSTM inputs
    mat_for_predict = np.column_stack((target_traffic,mat_compressed))                            ## shape(mat_for_predict) = (8594,3)
    sess.close()
    return mat_for_predict,target_traffic                                               ##  shape(target_traffic) = (8594,)
'''
a1,b,a,c = get_compressed_representation()
plt.plot(np.arange(len(a[:,3])),a[:,3],'b--',label = 'reconstruct traffic')
plt.plot(np.arange(len(c[:,3])),c[:,3],'r',label = 'ground truth traffic')
plt.xlim((0,1500))
plt.legend()
plt.show()
'''
...
def get_trainSet():
    ...
    return seq,res,xs                                                           ##shape(seq) = (8594-288, 288, 3)
                                                                                ##shape(res) = (8594-288, 288, 1)
                                                                                ##shape(xs) = (8594-288,)
...
if __name__ =='__main__':
    ...
    writer = tf.train.SummaryWriter("logs_5min", sess.graph)
    sess.run(tf.initialize_all_variables())
        ...
        for i in range(20):
            ...
            _, cost, state, pred = sess.run(
                [model.train_op, model.cost, model.cell_final_state, model.pred],feed_dict=feed_dict)
                                                                                ...
                                                                                ##shape(xs) = (8594-288,)
##test model2                                                                   ##shape(seq) = (8594-288, 288, 3)
    seq,res,xs = get_trainSet()                                                 ##shape(res) = (8594-288, 288, 1)
    feed_dict = {
                model.xs:seq[6000:8100:7].reshape(-1,TIME_STEPS,3)
                }

    res = res[6000:8100:7].reshape(-1,288,1)                                            ##the ground truth result to compare
    res = res[::40]
    ...
    res = res[:].flatten()
    pred = pred.flatten()
    print metrics.mean_squared_error(res,pred)              ##MSE = 0.0152017225855
    print metrics.mean_absolute_error(res,pred)             ##MAE = 0.0810971310964
