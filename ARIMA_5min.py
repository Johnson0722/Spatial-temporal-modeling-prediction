...
rng = pd.date_range('6/1/2014 00:00',periods = 8594,freq = '5Min')
traffic = normalization(traffic)
traffic_train = traffic[:6000]
traffic_test = traffic[6000:]                                                #type(traffic) = <class 'pandas.core.series.Series'>   
traffic_train = pd.Series(traffic_train,index = rng[:6000])
traffic_test = pd.Series(traffic_test,index = rng[6000:])
...
'''
model = ARMA(traffic_train, order=(3, 1)) 
result_arma = model.fit( disp=-1, method='css')
predict_ts_train = result_arma.predict()
predict_ts_test = result_arma.predict('6/21/2014 20:00:00','6/30/2014 20:05:00',dynamic = True)
...
traffic_test.to_csv('/home/johnson/tensorflow/pic/5min/result_5min/ARIMA_test_result')
predict_ts_test.to_csv('/home/johnson/tensorflow/pic/5min/result_5min/ARIMA_predict_result')
print metrics.mean_squared_error(traffic_test,predict_ts_test)              ##MSE = 0.0201995759071
print metrics.mean_absolute_error(traffic_test,predict_ts_test)             ##MAE = 0.106304066538
'''